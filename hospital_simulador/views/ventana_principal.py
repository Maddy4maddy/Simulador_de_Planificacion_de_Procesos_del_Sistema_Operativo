...
from views.ventana_historial import VentanaHistorial
from views.ventana_reporte import VentanaReporte
from views.ventana_reporte_simulacion import VentanaReporteSimulacion
...
class VentanaPrincipal:
    ...
    def crear_menu_principal(self):
        ...
        scroll_frame = tk.Frame(canvas, bg=EstilosHospital.COLORES["fondo"])
        ...
        separador = tk.Frame(scroll_frame, height=10, bg=EstilosHospital.COLORES["fondo"])
        separador.pack(fill="x")

        botones_frame = tk.Frame(scroll_frame, bg=EstilosHospital.COLORES["fondo"])
        ...
        botones = [
            ...
            ("Historial Ejecuciones", self.abrir_historial),
            ("Reporte Global", self.abrir_reporte),
        ...
        for texto, comando in botones:
            if "Eliminar" in texto or "Limpiar" in texto:
                btn = EstilosHospital.crear_boton_eliminar(botones_frame, texto, comando)
            elif "Salir" in texto:
                btn = EstilosHospital.crear_boton_gris(botones_frame, texto, comando)
            else:
                btn = EstilosHospital.crear_boton_rojo(botones_frame, texto, comando)
    ...
    def crear_lista_pacientes(self):
        ...
        columnas = ("ID", "Nombre", "Tipo", "Prioridad", "Llegada", "Rafaga",
                    "Restante", "Espera", "Gestiones", "Tiquete", "Estado")

        self.tree = ttk.Treeview(table_frame, columns=columnas, show="headings", height=8)

        anchos = [60, 180, 120, 80, 80, 100, 100, 100, 80, 100, 120]
    ...
    def crear_panel_estadisticas(self):
        ...
        for tipo, datos in EstilosHospital.TIPOS_PACIENTE.items():
            tk.Label(
                leyenda_frame, text=tipo,
                bg=datos["bg"], fg=datos["fg"],
                padx=8, pady=2,
    ...
    def actualizar_lista(self):
        ...
        for p in self.gestor.listar_pacientes():
            ...
            self.tree.insert("", "end", values=(
                ...
                self.formatear_tiempo(p.tiempo_espera),
                p.gestiones,
                tiquete_num,
                estado
            ), tags=tags)
    ...
    def abrir_registro(self):
        VentanaRegistro(self.root, self.gestor, self.actualizar_lista)

    def abrir_eliminar(self):
        VentanaEliminar(self.root, self.gestor, self.actualizar_lista)
    ...
    def ejecutar_mlq(self):
        ...
        resultado = self.simulation.ejecutar(configuracion, quantum=2)
        ...
        self.abrir_reporte_simulacion(resultado)
    ...
    def abrir_reporte(self):
        VentanaReporte(self.root, self.historial)

    def abrir_reporte_simulacion(self, resultado):
    ...
    def crear_config_mlq(self, parent):
        ...
        for cola, var in self.config_mlq_vars.items():
            ...
            tk.Label(fila, text=cola, bg=EstilosHospital.COLORES["fondo"]).pack(side="left")
